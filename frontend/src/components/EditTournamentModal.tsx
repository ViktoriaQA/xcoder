...
import { CalendarIcon, Edit, Trophy } from "lucide-react";
...
const EditTournamentModal = ({ open, onOpenChange, tournamentId, onSuccess }: EditTournamentModalProps) => {
  ...
  // Load tournament data when modal opens and tournamentId is provided
  useEffect(() => {
    if (open && tournamentId && token) {
      const fetchTournament = async () => {
        ...
        try {
          ...
          setFormData({
            ...
            min_participants: tournament.min_participants || 1,
          ...
          toast({
            ...
            description: t('tournaments.failedToLoadTournament'),
            variant: "destructive",
          });
          onOpenChange(false);
        } finally {
  ...
  }, [open, tournamentId, token, t, toast, onOpenChange]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!tournamentId) {
      toast({
        title: t('common.error'),
        description: t('tournaments.tournamentIdRequired'),
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          ...
          throw new Error(t('tournaments.notAuthorizedToEdit'));
        }
        if (response.status === 404) {
          throw new Error(t('tournaments.tournamentNotFound'));
        }
      ...
      toast({
        title: t('common.error'),
        description: error instanceof Error ? error.message : t('tournaments.failedToUpdateTournament'),
        variant: "destructive",
      });
    } finally {
      ...
      <DialogContent className="sm:max-w-[600px] bg-card/95 backdrop-blur-sm border-border/50">
        <DialogHeader>
          ...
          <DialogDescription className="font-mono text-sm">
            {t('tournaments.editTournamentDescription')}
          </DialogDescription>
        </DialogHeader>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    {t('tournaments.updateTournament')}
                  </>
                )}
              </Button>
            </DialogFooter>
